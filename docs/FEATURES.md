...
| Editor       | Native visual builder, nested sections, responsive design, reusable templates, drag/drop, undo/redo, media                            | Gutenberg compatibility, inline rich-text toolbar, globally synced patterns, dynamic widgets, collaboration |
...
Traffic starts at installation of the statistics migration. Only successful anonymous public HTML GET requests are counted; signed-in visits, previews, searches/query strings (except pagination), common bots, prefetch, errors, and non-document requests are excluded. Repeated requests count again: these are approximate page views, not unique visitors. Only aggregate daily path totals and site-wide hourly counts are kept; no IPs, cookies, visitor IDs, query strings, or referrers are recorded. Data older than the rolling 90-day window is deleted on the next counted view or statistics request. Content history may predate traffic collection. Statistics requires administrator access; other roles and anonymous requests cannot read it.


**Peak hours** shows total views for each hour of the day and highlights the busiest hour(s), with a Coordiation NativeSelect for Jakarta/WIB (UTC+7, default) or UTC. The selected reporting dates continue to use UTC; the hour selector only shifts hour-of-day labels. Hourly recording begins with migration `cms-010-hourly-statistics`; older daily counts are preserved but cannot be reconstructed into hourly counts. The collection start date and this limitation are shown in the card. No hourly data produces an empty state, not a fabricated peak.

**Page heatmap** compares the ten most viewed paths against every day in the selected period, using the existing daily records. Darker cells indicate more views and pale cells indicate zero. Full paths, dates and counts are available in tooltips and an accessible table; long periods scroll horizontally on narrow screens. This measures traffic intensity, not clicks, mouse movement, or scroll positions. Both new views follow the existing exclusions, administrator permissions, and rolling 90-day retention.

## Native page builder

Posts and Pages now include a docked Insert/Layers, canvas, and Content/Style/Page workspace with nested sections, responsive styles, media, undo/redo, and reusable JSON templates. Theme, wide, and blank canvas layouts are supported. See [Page Builder](PAGE-BUILDER.md) for usage, permissions, portability, and current limitations.
