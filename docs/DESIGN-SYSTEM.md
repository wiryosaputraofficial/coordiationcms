...
Statistics adapts the official [Interactive Chart](https://coordiation.com/docs/charts) registry retrieved on 2026-09-07 ([source](https://coordiation.com/r/interactive-chart.json)) into native Fullstack JavaScript. The registry pins ECharts 5.6.0; this CMS uses the patched ECharts 6.1.0 engine ([security advisory](https://github.com/advisories/GHSA-fgmj-fm8m-jvvx)) with the same component lifecycle. It uses an SVG renderer, lazy loading when visible, ResizeObserver, reduced-motion support, disposal on navigation, retry controls, and accessible data tables. Only the required line, bar, pie, and heatmap renderers are bundled. No React runtime or illustrative registry datasets are included. The original MIT notice is retained in `src/client/COORDIATION-CHART-LICENSE.txt`.

The Statistics toolbar renders shared `field`, `nativeSelect`, and `outlineButton` adapters directly from `src/client/ui.js`, using the official Label, NativeSelect and Button outline/medium recipes. The select keeps native keyboard and mobile picker behavior with a consistent Solar chevron; the refresh action uses the Solar restart icon. Controls share a 40 px height and an aligned baseline.

The page heatmap follows the Interactive Chart heatmap recipe with a continuous visual map, SVG cells, and a horizontally scrollable date grid. Peak-hours filters use the same shared NativeSelect and Label components as the period selector.
