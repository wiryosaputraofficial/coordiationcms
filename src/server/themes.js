...
const home = `<header class="site-header"><a class="site-logo" href="/"><img src="/coordiation-logo.png" alt="Coordiation">{{site.title}}</a><nav>{{#each menu}}<a href="{{href}}">{{label}}</a>{{/each}}</nav><form action="/"><input aria-label="Search posts" name="q" placeholder="Search stories…" value="{{query}}"><button>Search</button></form></header><main><section class="hero"><span class="eyebrow">THE JOURNAL</span><h1>{{site.title}}</h1><p>{{site.tagline}}</p></section><section class="post-grid">{{#each posts}}<article class="post-card">{{#if image}}<a href="/{{slug}}"><img src="{{image}}" alt="{{imageAlt}}"></a>{{/if}}<span class="eyebrow">{{category}} · {{date}}</span><h2><a href="/{{slug}}">{{title}}</a></h2><p>{{excerpt}}</p><a class="read-link" href="/{{slug}}">Read story <span>↗</span></a></article>{{else}}<p>No stories to show yet.</p>{{/each}}</section><nav class="pagination">{{#if previous}}<a href="{{previous}}">← Previous</a>{{/if}}{{#if next}}<a href="{{next}}">Next →</a>{{/if}}</nav></main><footer>{{site.footer}}</footer>`;
const single = `<header class="site-header"><a class="site-logo" href="/"><img src="/coordiation-logo.png" alt="Coordiation">{{site.title}}</a><nav>{{#each menu}}<a href="{{href}}">{{label}}</a>{{/each}}</nav><a href="/">← All stories</a></header><main class="single"><span class="eyebrow">{{post.category}} · {{post.date}}</span><h1>{{post.title}}</h1><p class="lead">{{post.excerpt}}</p><p class="byline">{{post.author}}{{#if post.readingTime}} · {{post.readingTime}} min read{{/if}}</p>{{#if post.image}}<img class="cover" src="{{post.image}}" alt="{{post.imageAlt}}">{{/if}}<div class="article-body">{{{content}}}</div>{{{comments}}}</main><footer>{{site.footer}}</footer>`;
